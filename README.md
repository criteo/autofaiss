...
| --file_format                | "npy"        | File format of the files in embeddings Can be either `npy` for numpy matrix files or `parquet` for parquet serialized tables or `npz` for one! compressed matrix (more than one archived array are skipped)|
